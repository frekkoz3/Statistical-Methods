...
curve(pnorm(x, 0, sqrt((3 / ((3 - 2) * n)))),
  add = TRUE, col = "red",
  lty = 2, lwd = 2)

plot(ecdf(sample_stat[3, ]), xlab = "y", main = "U(0,1)", cex.main = 1.5)
curve(pnorm(x, 1 / 2, sqrt(1 / (12 * n))),
  add = TRUE, col = "red",
  lty = 2, lwd = 2)
...
for (k in 1:4) {
  ...
  for (j in 1:length(n)) {
    hist(est[, k, j],
      nclass = 30, probability = T, xlab = "", xlim = c(-2, 6),
      main = label_est[k]
    )
...
hist(s2,
  breaks = 50, xlab = expression(s^2), probability = TRUE,
...
# plot s2 biased
hist(s2_b,
  breaks = 50, xlab = expression(s[b]^2), probability = TRUE,
